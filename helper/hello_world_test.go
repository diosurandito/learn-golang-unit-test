...
func BenchmarkHelloWorldTable(b *testing.B) {
	benchmarks := []struct {
		...
		{
			name:    "HelloWorld(Bayu)",
			request: "Bayu",
		},
		{
			name:    "HelloWorld(Odenk)",
			request: "Odenk",
		},
		{
			name:    "HelloWorld(Gilang)",
			request: "Gilang",
		},
	}
...
func BenchmarkHelloWorldSub(b *testing.B) {
	b.Run("Dio", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			HelloWorld("Dio")
		}
	})
	b.Run("Dito", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			HelloWorld("Dito")
		}
	})
...
func BenchmarkHelloWorldDito(b *testing.B) {
	for i := 0; i < b.N; i++ {
		HelloWorld("Dito")
	}
}
